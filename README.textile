...
...add the sign in/out (connect) link somewhere:

<pre>
  <%= facebook_link :for => :user %>
</pre>

or more explicit, something like:
...
* *Make sign in helper :for option assumed* if only one facebook_connectable reosurce is available in the app.
