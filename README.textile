...
<pre>
  $ ./script/generate devise_facebook_connectable
...
..if you haven't already: *"Create Facebook Application":http://facebook.com/developers/createapp.php*
...
NOTE: This one _should_ work, but right now not stable because of @Facebooker::Session::SessionExpired@ gets raised all the time. >=(
...
* *Expired sessions* should be handled more seamless. before_facebook_connect-hook dont' work very well @facebook.users.getInfo (0) Facebooker::Session::SessionExpired: Session key invalid or no longer valid@. =(
