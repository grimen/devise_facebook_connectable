...
* *"devise":http://github.com/plataformatec/devise* for authentication - based on "warden":http://github.com/hassox/warden/.
  ...
  en:
    devise:
      facebook_sessions:
        ...
        unauthenticated: "You need to sign in (connect) before continuing."
        invalid: "Could not login. Invalid account."
        timeout: "Facebook session expired., please sign in again to continue."
        actions:
          login: "Connect"
          logout: "Logout"
</pre>
...
* *Make login helper :for option assumed* if only one facebook_connectable reosurce is available in the app.
* *Ensure no module clashes* with Devise core modules is a design goal, but have only tested @Tracking@ so far for no reason.
* *@confirmable@ should be ignored for FBC-created accounts* in case it's enabled (for @authenticatable@ module).
...
* *Specs/Features* would be great. Skipped tests so far as I built this using experimenting as I wasn't sure how Devise and Warden was working - trial-n-error style. Would appreciate any help on this. Looking into using Cucumber + "watircuke":http://github.com/richdownie/watircuke now.
